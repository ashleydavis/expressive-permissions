...
This doc explains how to read a permission decision, find gaps, and add a rule (the workflow behind `/permissions:check`, `/permissions:allow`, and `/permissions:deny`).
...
From the snippet, extract for every sub-command: the sub-command string, its decision, the rule that fired (`file:line`) and reason if any, and whether it was a **NOMATCH** (every rule abstained).
...
A **NOMATCH** / `no_rule_match` means no rule had anything to say, so the engine fell back to its default. These are the gaps to close with a new rule. See [Finding tool calls that no rule matched](TROUBLESHOOTING.md#finding-tool-calls-that-no-rule-matched).
...
A command can move between classes depending on its flags and arguments (a read-only tool with an in-place-edit flag is mutating; a path argument outside the project is riskier than one inside it). Judge the specific invocation in the snippet, not just the command name.
...
- Re-run the command, or re-analyse it, to confirm the new decision. Configuration is reloaded automatically on the next hook run. See [Confirming the config is loaded](TROUBLESHOOTING.md#confirming-the-config-is-loaded).

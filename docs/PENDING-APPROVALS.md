...
This doc explains the Markdown files written when a tool call returns `ask`, so you can inspect the decision while approving.
...
| User denies the prompt | File remains until stale cleanup (1 day) or manual delete |
...
5. **Parsed command tree**: ASCII tree of parsed sub-commands with labeled `cwd`, `env`, `decision`, `rule`, and `reason` lines per sub-command

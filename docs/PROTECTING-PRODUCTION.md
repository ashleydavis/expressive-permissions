...
This doc gives recipes for locking down AWS CLI and kubectl (and similar) with permissions rules.
...
Match on the active Kubectl context via the `--context` argument. Requires a kubectl [command descriptor](CONFIGURATION.md#command-descriptor-files-commands) so `context` is parsed as a value-taking flag. `not:` cannot invert flag *values*, so match non-sandbox contexts with a regex on `options.context` instead:
...
bash:
  kubectl:
    ...
    # Any explicit non-sandbox --context value: apply these rules
    - options:
        context: "/^(?!sandbox$)/"
      rules:
        # Allow read-only operations
...
This matches when `--context` is passed explicitly, which is common in scripts and pipelines. If the active context was set with `kubectl config use-context` and commands run without the flag, these rules will not match; for that case, match on kubeconfig contents instead (see the previous section).

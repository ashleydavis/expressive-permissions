...
Worked AST examples for every construct live under [`examples/bash/`](../examples/bash) (compact, Bash-only) and [`examples/ast/`](../examples/ast) (full tool-call AST), each paired with a `.md` Mermaid diagram. Both sets are verified by the smoke suites.

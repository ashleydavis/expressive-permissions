...
test("bare xargs: empty options and empty child binary", () => {
    const result = parseXargsCommand("xargs");
...
test("-n 1: options.n === '1', child is the subcommand", () => {
    const result = parseXargsCommand("xargs -n 1 rm");
    expect(result.options["n"]).toBe("1");
    expect(result.child.binary).toBe("rm");
});

test("-P 4: options.P === '4'", () => {
    const result = parseXargsCommand("xargs -P 4 grep");
    expect(result.options["P"]).toBe("4");
    expect(result.child.binary).toBe("grep");
});

test("-I{} (value attached): options.I === '{}'", () => {
    const result = parseXargsCommand("xargs -I{} cp {} /dest");
    expect(result.options["I"]).toBe("{}");
    expect(result.child.binary).toBe("cp");
});
...
test("-n1 (value attached, no space): options.n === '1'", () => {
    const result = parseXargsCommand("xargs -n1 echo");
    expect(result.options["n"]).toBe("1");
    expect(result.child.binary).toBe("echo");
});
...
test("-0: boolean flag options['0'] === true", () => {
    const result = parseXargsCommand("xargs -0 rm");
    expect(result.options["0"]).toBe(true);
    expect(result.child.binary).toBe("rm");
});

test("-0t: bundled boolean flags both set to true", () => {
    const result = parseXargsCommand("xargs -0t grep");
    expect(result.options["0"]).toBe(true);
...
test("--max-args 5: options['max-args'] === '5'", () => {
    const result = parseXargsCommand("xargs --max-args 5 echo");
    expect(result.options["max-args"]).toBe("5");
    expect(result.child.binary).toBe("echo");
});

test("--replace={}: options['replace'] === '{}'", () => {
    const result = parseXargsCommand("xargs --replace={} cp {} /dest");
...
test("--no-run-if-empty: options['no-run-if-empty'] === true", () => {
    const result = parseXargsCommand("xargs --no-run-if-empty rm");
    expect(result.options["no-run-if-empty"]).toBe(true);
    expect(result.child.binary).toBe("rm");
});

test("--verbose: options['verbose'] === true", () => {
    const result = parseXargsCommand("xargs --verbose grep");
...
test("--delimiter=,: options['delimiter'] === ','", () => {
    const result = parseXargsCommand("xargs --delimiter=, echo");
...
test("xargs 2>/dev/null grep: redirect before subcmd is skipped, child is grep", () => {
    const result = parseXargsCommand("xargs 2>/dev/null grep");
    expect(result.child.binary).toBe("grep");
});
...
test("xargs grep -l pattern: child has options.l and correct binary", () => {
    const result = parseXargsCommand('xargs grep -l "pattern"');
    expect(result.child.binary).toBe("grep");
    expect(result.child.options["l"]).toBeDefined();
});

test("xargs rm -rf: child binary is rm with options r and f", () => {
    const result = parseXargsCommand("xargs rm -rf");
    expect(result.child.binary).toBe("rm");
    expect(result.child.options["r"]).toBe(true);
    expect(result.child.options["f"]).toBe(true);
});
...
test("-n 1 -P 4 grep: both options parsed, child is grep", () => {
    const result = parseXargsCommand("xargs -n 1 -P 4 grep");
    expect(result.options["n"]).toBe("1");
    expect(result.options["P"]).toBe("4");
    expect(result.child.binary).toBe("grep");
});

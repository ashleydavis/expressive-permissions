...
// ---------------------------------------------------------------------------
// PROTECTING-PRODUCTION.md: pos array with wildcard first element
// e.g. aws: pos: ["*", "describe-*"]
// ---------------------------------------------------------------------------

test("bash pos array with wildcard: pos[0]=* matches any service, pos[1]=describe-* matches operation", () => {
    const yaml = `
bash:
  aws:
    - pos:
...
`;
    withYamlFixtures(null, yaml, (rules) => {
        ...
        expect(decide(rules[1], makeCommand("aws", ["ec2", "delete-instance"]))).toBe("deny");
        expect(decide(rules[0], makeCommand("aws", ["ec2", "run-instances"]))).toBe("abstain");
    });
});

// ---------------------------------------------------------------------------
// PROTECTING-PRODUCTION.md: env regex with negative lookahead
// e.g. AWS_PROFILE: /^(?!sandbox$)/
// ---------------------------------------------------------------------------

test("bash env regex negated lookahead: matches any profile except sandbox", () => {
    const yaml = `
bash:
  aws:
    env:
...
`;
    withYamlFixtures(null, yaml, (rules) => {
        expect(decide(rules[0], makeCommand("aws", ["ec2", "delete-instance"]), makeEnv("/x", true, { AWS_PROFILE: "prod" }))).toBe("deny");
        expect(decide(rules[0], makeCommand("aws", ["ec2", "delete-instance"]), makeEnv("/x", true, { AWS_PROFILE: "staging" }))).toBe("deny");
        expect(decide(rules[0], makeCommand("aws", ["ec2", "delete-instance"]), makeEnv("/x", true, { AWS_PROFILE: "sandbox" }))).toBe("abstain");
    });
});

// ---------------------------------------------------------------------------
// PROTECTING-PRODUCTION.md: args object combined with pos (kubectl --context)
// ---------------------------------------------------------------------------

test("bash args object + pos combined: kubectl context flag + subcommand", () => {
    const yaml = `
bash:
  kubectl:
    - args:
...
`;
    withYamlFixtures(null, yaml, (rules) => {
        expect(decide(rules[0], makeCommand("kubectl", "delete", { context: "prod-cluster" }))).toBe("deny");
        expect(decide(rules[0], makeCommand("kubectl", "delete", { context: "prod-east" }))).toBe("deny");
        expect(decide(rules[0], makeCommand("kubectl", "get", { context: "prod-cluster" }))).toBe("abstain");
        expect(decide(rules[1], makeCommand("kubectl", "delete", { context: "sandbox-cluster" }))).toBe("allow");
    });
});
...
// ---------------------------------------------------------------------------
// README table: aws list-* and get-* → allow (in addition to describe-*)
// ---------------------------------------------------------------------------

test("README table: aws list-* and get-* operations → allow", () => {
    const yaml = `
bash:
  aws:
    - pos:
...
`;
    withYamlFixtures(null, yaml, (rules) => {
        ...
        expect(strictest(makeCommand("aws", ["ec2", "describe-instances"]))).toBe("allow");
        expect(strictest(makeCommand("aws", ["s3", "list-objects"]))).toBe("allow");
        expect(strictest(makeCommand("aws", ["iam", "get-user"]))).toBe("allow");
        expect(strictest(makeCommand("aws", ["ec2", "run-instances"]))).toBe("abstain");
        expect(strictest(makeCommand("aws", ["s3", "put-object"]))).toBe("abstain");
    });
});

// ---------------------------------------------------------------------------
// README table: aws create-* and modify-* on prod profile → deny
// ---------------------------------------------------------------------------

test("README table: aws create-* modify-* on non-sandbox profile → deny", () => {
    const yaml = `
bash:
  aws:
    - env:
      ...
      decide: deny
      reason: Destructive operations blocked on non-sandbox profiles
`;
    withYamlFixtures(null, yaml, (rules) => {
        const prodEnv = makeEnv("/project", true, { AWS_PROFILE: "prod" });
        const stagingEnv = makeEnv("/project", true, { AWS_PROFILE: "staging" });
        const sandboxEnv = makeEnv("/project", true, { AWS_PROFILE: "sandbox" });

        expect(decide(rules[0], makeCommand("aws", ["ec2", "create-instance"]), prodEnv)).toBe("deny");
        expect(decide(rules[0], makeCommand("aws", ["ec2", "delete-instance"]), prodEnv)).toBe("deny");
        expect(decide(rules[0], makeCommand("aws", ["rds", "modify-db-instance"]), stagingEnv)).toBe("deny");
        expect(decide(rules[0], makeCommand("aws", ["ec2", "create-instance"]), sandboxEnv)).toBe("abstain");
        expect(decide(rules[0], makeCommand("aws", ["ec2", "describe-instances"]), prodEnv)).toBe("abstain");
    });
});

// ---------------------------------------------------------------------------
// README table: kubectl get, describe, logs → always allow
// ---------------------------------------------------------------------------

test("README table: kubectl read-only operations always allow", () => {
    const yaml = `
bash:
  kubectl:
    - pos-in:
...
`;
    withYamlFixtures(null, yaml, (rules) => {
        expect(decide(rules[0], makeCommand("kubectl", ["get", "pods"]))).toBe("allow");
        expect(decide(rules[0], makeCommand("kubectl", ["describe", "pod", "my-pod"]))).toBe("allow");
        expect(decide(rules[0], makeCommand("kubectl", ["logs", "my-pod"]))).toBe("allow");
        expect(decide(rules[0], makeCommand("kubectl", ["apply", "-f", "manifest.yaml"]))).toBe("abstain");
        expect(decide(rules[0], makeCommand("kubectl", ["delete", "pod", "my-pod"]))).toBe("abstain");
    });
});

// ---------------------------------------------------------------------------
// README table: kubectl apply, delete, exec targeting production → deny
// ---------------------------------------------------------------------------

test("README table: kubectl apply delete exec on prod cluster → deny", () => {
    const yaml = `
bash:
  kubectl:
    - args:
      ...
      pos-in:
        ...
        - exec
      decide: deny
      reason: Destructive operations blocked on production clusters
`;
    withYamlFixtures(null, yaml, (rules) => {
        expect(decide(rules[0], makeCommand("kubectl", ["delete", "pod", "my-pod"], { context: "prod-east" }))).toBe("deny");
        expect(decide(rules[0], makeCommand("kubectl", ["apply", "-f", "manifest.yaml"], { context: "prod-cluster" }))).toBe("deny");
        expect(decide(rules[0], makeCommand("kubectl", ["exec", "-it", "my-pod"], { context: "prod-west" }))).toBe("deny");
        expect(decide(rules[0], makeCommand("kubectl", ["delete", "pod", "my-pod"], { context: "staging-cluster" }))).toBe("abstain");
        expect(decide(rules[0], makeCommand("kubectl", ["get", "pods"], { context: "prod-east" }))).toBe("abstain");
    });
});

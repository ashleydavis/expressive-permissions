...
test("exportRule: non-export binary → abstain", () => {
    const result = exportRule(makeCommand("echo", makeArgs({}), ["FOO=bar"], {}), makeEnv("/start", true, {}), dummyCall);
    expect(result.decision.action).toBe("abstain");
    expect(result.env).toBeUndefined();
});

test("exportRule: export with no KEY=VALUE tokens → abstain", () => {
    const result = exportRule(makeCommand("export", makeArgs({}), [], {}), makeEnv("/start", true, {}), dummyCall);
    expect(result.decision.action).toBe("abstain");
    expect(result.env).toBeUndefined();
});

test("exportRule: export with bare name (no =) → abstain", () => {
    const result = exportRule(makeCommand("export", makeArgs({}), ["FOO"], {}), makeEnv("/start", true, {}), dummyCall);
    expect(result.decision.action).toBe("abstain");
    expect(result.env).toBeUndefined();
});
...
test("exportRule: decision is always abstain on match", () => {
    const result = exportRule(makeCommand("export", makeArgs({}), ["FOO=bar"], {}), makeEnv("/start", true, {}), dummyCall);
    expect(result.decision.action).toBe("abstain");
});
...
test("exportRule: multiple exports all appear in persistent env", () => {
    const result = exportRule(makeCommand("export", makeArgs({}), ["A=1", "B=2"], {}), makeEnv("/start", true, {}), dummyCall);
    expect(result.env?.env.A).toBe("1");
    expect(result.env?.env.B).toBe("2");
});

test("exportRule: existing env vars are preserved", () => {
    const result = exportRule(makeCommand("export", makeArgs({}), ["NEW=val"], {}), makeEnv("/start", true, { EXISTING: "keep" }), dummyCall);
...
test("exportRule: exported var overrides existing var with same name", () => {
    const result = exportRule(makeCommand("export", makeArgs({}), ["FOO=new"], {}), makeEnv("/start", true, { FOO: "old" }), dummyCall);
    expect(result.env?.env.FOO).toBe("new");
});

test("exportRule: value containing = is captured correctly", () => {
    const result = exportRule(makeCommand("export", makeArgs({}), ["CONN=host=localhost"], {}), makeEnv("/start", true, {}), dummyCall);
...
test("exportRule: cwd is preserved in returned env", () => {
    const result = exportRule(makeCommand("export", makeArgs({}), ["FOO=bar"], {}), makeEnv("/mydir", true, {}), dummyCall);
